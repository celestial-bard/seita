...
import torch
import numpy as np
from av2.geometry import interpolate
from av2.map.map_api import ArgoverseStaticMap

from .av2_utils import load_scenario, object_type_to_int
...
class Av2Extractor:
    ...
    def process(self, raw_path: str, agent_id=None) -> Dict[str, Union[torch.Tensor, str]]:
        scenario_df, av_map, scenario_id = load_scenario(raw_path)
        city: str = scenario_df["city"].values[0]
        agent_id: str = scenario_df["focal_track_id"].values[0]

        local_scenario_df = scenario_df[scenario_df["track_id"] == agent_id].iloc
        origin = torch.tensor(
            [local_scenario_df[50]["position_x"], local_scenario_df[50]["position_y"]], dtype=torch.float
        )
        theta = torch.tensor([local_scenario_df[50]["heading"]], dtype=torch.float)
        rotate_mat = torch.tensor([
            [torch.cos(theta), -torch.sin(theta)],
            [torch.sin(theta), torch.cos(theta)],
        ])

        timestamps = list(np.sort(scenario_df["timestep"].unique()))
        cur_scenario_df = scenario_df[scenario_df["timestep"] == timestamps[50]]
        actor_ids = list(cur_scenario_df["track_id"].unique())
        cur_pos = torch.from_numpy(cur_scenario_df[["position_x", "position_y"]].values).float()
        ...
        scenario_df = scenario_df[scenario_df["track_id"].isin(actor_ids)]
        ...
        for actor_id, actor_df in scenario_df.groupby("track_id"):
            node_idx = actor_ids.index(actor_id)
            node_steps = [timestamps.index(ts) for ts in actor_df["timestep"]]
            object_type = object_type_to_int(actor_df["object_type"].values[0], 'o2i')

            x_attr[node_idx, 0] = object_type
            x_attr[node_idx, 1] = actor_df["object_category"].values[0]
            x_attr[node_idx, 2] = object_type_to_int(actor_df["object_type"].values[0], 'o2if')
            x_track_horizon[node_idx] = node_steps[-1] - node_steps[0]

            padding_mask[node_idx, node_steps] = False
            if padding_mask[node_idx, 50] or object_type in self.ignore_type:
                padding_mask[node_idx, 51:] = True
        ...
        (
            ...
            lane_attr,
            lane_padding_mask,
        ) = self.get_lane_features(av_map, origin, origin, rotate_mat, self.radius)
        
        if self.remove_outlier_actors:
            lane_samples = lane_positions[:, ::1, :2].view(-1, 2)
            nearest_dist = torch.cdist(x[:, 50, :2], lane_samples).min(dim=1).values
        ...
        x_velocity_diff = x_velocity[:, :].clone()

        x[:, :50] = torch.where(
            (padding_mask[:, 50].unsqueeze(-1) | padding_mask[:, :50]).unsqueeze(-1),
            torch.zeros(num_nodes, 50, 2),
            x[:, :50] - x[:, 50].unsqueeze(-2),
        )
        x[:, 51:] = torch.where(
            (padding_mask[:, 50:109] | padding_mask[:, 51:]).unsqueeze(-1),
            torch.zeros(num_nodes, 59, 2),
            x[:, 51:] - x[:, 50:109],
        )
        x[:, 50] = torch.zeros(num_nodes, 2)

        x_velocity_diff[:, 1:] = torch.where(
            (padding_mask[:, :109] | padding_mask[:, 1:]),
            torch.zeros(num_nodes, 109),
            x_velocity_diff[:, 1:] - x_velocity_diff[:, :109],
        )
        ...
        return {
            "actor_pos": x,
            "actor_attr": x_attr,
            "actor_real_pos": x_positions,
            "actor_centers": x_ctrs,
            "actor_angles": x_heading,
            "actor_velocity": x_velocity,
            "actor_velocity_diff": x_velocity_diff,
            "actor_padding_mask": padding_mask,
            ...
            "origin_point": origin.view(-1, 2),
            "theta_at_origin": theta,
    ...
    def get_lane_features(
        ...
        for segment in lane_segments:
            lane_centerline, lane_width = interpolate.compute_midpoint_line(
            ...
            # get lane attrs
            lane_type = object_type_to_int(segment.lane_type, 'l2i')
