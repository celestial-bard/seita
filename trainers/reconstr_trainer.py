...
class Trainer(pl.LightningModule):
    ...
    def cal_loss(self, out, data):
        ...
        y, y_others = data["actor_pos"][:, 0, :50, :], data["actor_pos"][..., 1:, :50, :]

        l2_norm = torch.norm(y_hat[..., :2] - y.unsqueeze(1), dim=-1).sum(dim=-1)
        best_mode = torch.argmin(l2_norm, dim=-1)
        y_hat_best = y_hat[torch.arange(y_hat.shape[0]), best_mode]

        agent_reg_loss = F.huber_loss(y_hat_best[..., :2], y)
        agent_cls_loss = F.cross_entropy(pi, best_mode.detach())

        others_reg_mask = ~data["actor_padding_mask"][:, 1:, :50]
        others_reg_loss = F.huber_loss(
    ...
    def validation_step(self, data, batch_idx):
        out = self(data)
        losses = self.cal_loss(out, data)
        metrics = self.val_metrics(out, data["actor_pos"][:, 0, :50, :])
