...
def plot_reconstruction(data, out, save_path="./test.pdf"):
    ...
    # agents history plot
    key_padding_mask = data["actor_key_padding_mask"][0].cpu()
    x = data["actor_pos"][0][..., 50:, :].cpu()[~key_padding_mask]
    x_centers = data["actor_centers"][0].cpu()[~key_padding_mask]
    x_padding_mask = data["actor_padding_mask"][0].cpu()[~key_padding_mask]
    x_attr = data["actor_attr"][0].cpu()[~key_padding_mask]

    # agent future plot
    y = data["actor_pos"][0][..., :50, :].cpu()
    y_padding_mask = data["actor_padding_mask"][0, :, :50]
    y += x_centers[:, None, :]
