...
from layers.recon_agent_enc_layer import AgentEmbeddingLayer
from layers.recon_lane_enc_layer import LaneEmbeddingLayer
from neural.nn.self_attn import Block
...
class MaskedAE(nn.Module):
    def __init__(
        ...
        self.future_embed = AgentEmbeddingLayer(3, embed_dim // 4, drop_path_rate=drop_path)
        ...
        self.future_pred = nn.Linear(embed_dim, history_steps * 2)
        self.history_pred = nn.Linear(embed_dim, future_steps * 2)
    ...
    def forward(self, data):
        future_padding_mask = data["actor_padding_mask"][:, :, :50]
        future_feat = torch.cat([data["actor_pos"][..., :50, :], ~future_padding_mask[..., None]], dim=-1)
        ...
        hist_padding_mask = data["actor_padding_mask"][:, :, 50:]
        hist_feat = torch.cat(
            [
                data["actor_pos"][..., 50:, :],
                data["actor_velocity_diff"][..., 50:, None],
        ...
        actor_type_embed = self.actor_type_embed[data["actor_attr"][..., 2].long()]
        ...
        x_centers = torch.cat(
            [data["actor_centers"], data["actor_centers"], data["lane_centers"]], dim=1
        )
        angles = torch.cat(
            [
                data["actor_angles"][..., 50],
                data["actor_angles"][..., 50],
                data["lane_angles"],
            ],
            dim=1,
        ...
        hist_pred_mask = ~data["actor_key_padding_mask"]

        # print(decoder_hist_token.shape, hist_tokens.shape, hist_pred_mask.shape)
        ...
        future_pred_mask = ~data["actor_key_padding_mask"]
        ...
        hist_token = x_decoder[:, N : 2 * N].reshape(-1, self.embed_dim)
        future_token = x_decoder[:, :N].reshape(-1, self.embed_dim)
        ...
        # hist pred loss
        x_hat = self.history_pred(hist_token).view(-1, 60, 2)
        x = (data["actor_real_pos"][..., 50:, :] - data["actor_centers"].unsqueeze(-2)).view(-1, 60, 2)
        x_reg_mask = ~data["actor_padding_mask"][:, :, 50:]
        x_reg_mask[~hist_pred_mask] = False
        x_reg_mask = x_reg_mask.view(-1, 60)
        hist_loss = F.l1_loss(x_hat[x_reg_mask], x[x_reg_mask])

        # future pred loss
        y_hat = self.future_pred(future_token).view(-1, 50, 2)  # B*N, 120
        y = data["actor_pos"][..., :50, :].view(-1, 50, 2)
        reg_mask = ~data["actor_padding_mask"][:, :, :50]
        reg_mask[~future_pred_mask] = False
        reg_mask = reg_mask.view(-1, 50)
        ...
        if not self.training:
            out["x_hat"] = x_hat.view(B, N, 60, 2)
            out["y_hat"] = y_hat.view(1, B, N, 50, 2)
