...
from layers.recon_dec_layer import MultimodalDecoder
...
class Reconstructor(nn.Module):
    def __init__(
        ...
        self.decoder = MultimodalDecoder(embed_dim, historical_steps)
        self.dense_predictor = nn.Sequential(
            nn.Linear(embed_dim, embed_dim * 2), 
            nn.ReLU(), 
            nn.Linear(embed_dim * 2, historical_steps * 2)
        )

        self.his_steps = historical_steps
    ...
    def forward(self, data):
        hist_padding_mask = data["actor_padding_mask"][:, :, self.his_steps:]
        hist_key_padding_mask = data["actor_key_padding_mask"]
        hist_feat = torch.cat([
            data["actor_pos"][..., self.his_steps:, :],
            data["actor_velocity_diff"][..., self.his_steps:, None],
        ...
        actor_feat = self.hist_embed(hist_feat[~hist_feat_key_padding].permute(0, 2, 1).contiguous())
        ...
        lane_normalized = torch.cat([lane_normalized, ~lane_padding_mask[..., None]], dim=-1)
        ...
        x_centers = torch.cat([data["actor_centers"], data["lane_centers"]], dim=1)
        angles = torch.cat([data["actor_angles"][:, :, self.his_steps], data["lane_angles"]], dim=1)
        ...
        key_padding_mask = torch.cat([data["actor_key_padding_mask"], data["lane_key_padding_mask"]], dim=1)
        ...
        y_hat_others = self.dense_predictor(x_others).view(B, -1, self.his_steps, 2)
